def get_dist(p1, p2):
        ...
        # 층이 바뀌면, elevator 연산이 추가되어야함
        c_elevator_dist = 0
        n_elevator_dist = 0

        if p1[0] != p2[0]:
            # p1과 p2에 대한 엘리베이터 위치 return
            c_elevator, n_elevator = select_elevator(p1, p2)

            # p1과 p2에 대한 엘리베이터와 p1, p2에 대한 거리
        ...
        if abs(p2[0]>p1[0]): return 2*abs(p1[0] - p2[0]) + abs(p1[1] - p2[1]) + abs(p1[2] - p2[2]) + c_elevator_dist + n_elevator_dist
        elif abs(p2[0]<p1[0]): return abs(p1[0] - p2[0]) + abs(p1[1] - p2[1]) + abs(p1[2] - p2[2]) + c_elevator_dist + n_elevator_dist
        else: return abs(p1[0] - p2[0]) + abs(p1[1] - p2[1]) + abs(p1[2] - p2[2])
